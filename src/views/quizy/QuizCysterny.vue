  ...
  <div>
    ...
    <section class="bg-gradient-to-r from-primary to-orange-600 text-white py-8 px-8">
      <div class="mx-auto max-w-[1440px]">
        <h1 class="text-3xl md:text-4xl font-bold">Quiz ADR - Cysterny</h1>
      </div>
    </section>
      ...
      <div class="mx-auto max-w-[1000px]">
        <div class="mb-8 flex justify-between items-center">
          <h2 class="text-2xl font-bold text-text-main">
            {{ selectedQuizId ? availableQuizzes.find(q => q.id === selectedQuizId)?.name : 'Quizy Cysterny' }}
          </h2>
          <button
            @click="logout"
        ...
        <div v-if="!selectedQuizId" class="space-y-6">
          <div class="bg-primary/10 rounded-lg p-6 mb-8">
            ...
            <p class="text-gray-600 text-center">Dostępnych jest 6 quizów specjalistycznych Cysterny</p>
          </div>
...
const STORAGE_KEY = 'quiz_cysterny_auth'
...
const availableQuizzes = ref([
  { id: 1, name: 'Cysterny 1', completed: false, score: null },
  { id: 2, name: 'Cysterny 2', completed: false, score: null },
  { id: 3, name: 'Cysterny 3', completed: false, score: null },
  { id: 4, name: 'Cysterny 4', completed: false, score: null },
  { id: 5, name: 'Cysterny 5', completed: false, score: null },
  { id: 6, name: 'Cysterny 6', completed: false, score: null }
])

// Baza pytań - Quiz 1
const quiz1Questions = [
  {
    question: 'Co to jest układ do opróżniania cysterny?',
    correctAnswer: 2,
    answers: [
      { letter: 'A', text: 'układ wydechowy, zabezpieczony przed iskrzeniem' },
      { letter: 'B', text: 'sposób postępowania przy opróżnianiu zbiorników' },
      { letter: 'C', text: 'ustawienie w kolejności armatury, koniecznej do opróżnienia zbiornika (pompa, urządzenie mierzące, zawory denne i spustowe)' }
    ]
  },
  {
    question: 'Instrukcja pisemna powinna:',
    correctAnswer: 2,
    answers: [
      { letter: 'A', text: 'udzielać wskazówek dla lekarza, który udziela pomocy zatrutym w wyniku wypadku' },
      { letter: 'B', text: 'znajdować się w kabinie kierowcy, przez okres dwóch tygodni po dokonanym przewozie' },
      { letter: 'C', text: 'podawać dodatkowe wskazówki dla członków załogi, co do ewentualnego stosowania środków ochrony indywidualnej' }
    ]
  },
  {
    question: 'Tablice z numerami rozpoznawczymi wymagane są:',
    correctAnswer: 1,
    answers: [
      { letter: 'A', text: 'w pojazdach przejeżdżających z towarem niebezpiecznym przez tunele' },
      { letter: 'B', text: 'do oznakowania pojazdów przewożących towary luzem' },
      { letter: 'C', text: 'w przypadku przewozu zgodnie z 1.1.3.6., określonego dopuszczalnym limitem ładunku' }
    ]
  },
  {
    question: 'Jakie są trzy główne czynniki niezbędne do powstania pożaru?',
    correctAnswer: 1,
    answers: [
      { letter: 'A', text: 'azot, benzyna i źródło zapłonu' },
      { letter: 'B', text: 'powietrze (tlen), paliwo i źródło zapłonu' },
      { letter: 'C', text: 'propan, butan i źródło zapłonu' }
    ]
  },
  {
    question: 'Z jakiego dokumentu związanego z przewozem kierowca pojazdu cysterny może dowiedzieć się do kiedy jest ważne badanie pojazdu cysterny?',
    correctAnswer: 2,
    answers: [
      { letter: 'A', text: 'z zaświadczenia o przeszkoleniu kierowców pojazdów przewożących towary niebezpieczne' },
      { letter: 'B', text: 'w instrukcjach pisemnych' },
      { letter: 'C', text: 'z ostatniego protokołu, z badań przeprowadzonych przez inspektora transportowego Dozoru technicznego jeżeli ten dokument lub kopia znajduje się w pojeździe' }
    ]
  },
  {
    question: 'W jakim dokumencie związanym z przewozem, kierowca pojazdu cysterny może znaleźć informacje o numerach UN przewożonych towarów, które powinny być umieszczone na tablicach barwy pomarańczowej?',
    correctAnswer: 1,
    answers: [
      { letter: 'A', text: 'w pisemnej instrukcji' },
      { letter: 'B', text: 'w załączniku A umowy ADR' },
      { letter: 'C', text: 'w załączniku B umowy ADR' }
    ]
  }
]
...
const finishQuiz = async () => {
  ...
  try {
    const { error } = await supabase
      ...
      .insert([
        {
          quiz_type: `cysterny_${selectedQuizId.value}`,
